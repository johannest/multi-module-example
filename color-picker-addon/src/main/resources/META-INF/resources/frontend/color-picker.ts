// NOTE! This copied directly from https://github.com/TatuLund/ColorPicker and just used to demonstrate
// how to have a separate add-on project in multi-module Maven Vaadin project
...
export class ColorPicker extends ThemableMixin(LitElement) {
  ...
  static get styles() {
    ...
    return css`
		...
		#colorpicker {
          // using variable from the custom theme
			color: var(--my-custom-variable);
		...
		#colorpicker:focus-visible {
          // using variable from the custom theme
          box-shadow: 0 0 0 2px var(--my-custom-variable);
		    outline: unset;
		}
		#colorpicker(not([disabled])):hover {
    		background: var(--my-custom-variable);
		}
        ...
        #combobox {
	        flex-grow: 1;
            background-color: var(--my-custom-variable);
        }
    `;
  }
